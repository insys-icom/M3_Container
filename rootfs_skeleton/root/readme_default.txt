...
Set up networking
===================================
This container has one ethernet interface which is bridged to one of the routers IP nets. The interface already got an IP address, it still lacks the correct default route. Enable and modify it permanently:
    # vi /bin/start_net.sh


Configure a DNS server address
===================================
Most of the time the router will be the DNS server as well. Modify the IP address of the DNS server:
    # vi /etc/resolv.conf
